...
void compute_Ad(double Ad[17000], double Ap[17000], double An[17000], double As[17000], double Ae[17000], double Aw[17000], double d[17000])
{
    Ad[1] = Ap[1]*d[1] + An[1]*d[2] + Ae[1]*d[129];

    for(int i = 2; i<=128; i++)
    {
        Ad[i] = As[i]*d[i-1] + Ap[i]*d[i] + An[i]*d[i+1] + Ae[i]*d[i+128];
    }

    for(int i = 129; i <= 128*127; i++)
    {
        Ad[i] = Aw[i]*d[i-128] + As[i]*d[i-1] + Ap[i]*d[i] + An[i]*d[i+1] + Ae[i]*d[i+128];
    }

    for(int i = 128*127+1; i <= 128*128-1; i++)
    {
        Ad[i] = Aw[i]*d[i-128] + As[i]*d[i-1] + Ap[i]*d[i] + An[i]*d[i+1];
    }

    Ad[128*128] = Aw[128*128]*d[128*128-128] + As[128*128]*d[128*128-1] + Ap[128*128]*d[128*128];
}
...
void vector_An_As_Ae_Aw(double An[17000], double As[17000], double Ae[17000], double Aw[17000])
{
...
void compute_SIP_Lp_Lw_Ls_Un_Ue(double alf, double Lp[17000], double Ls[17000], double Lw[17000], double Un[17000], double Ue[17000], double Ap[17000], double An[17000], double As[17000], double Ae[17000], double Aw[17000]) {
    for (int i = 1; i <= 128 * 128; i++) 
    {
        Lp[i] = 0.0;
        Ls[i] = 0.0;
    ...
    for (int i = 2; i <= 128; i++) 
    {
        Ls[i] = As[i] / (1 + alf * Ue[i - 1]);
        Lp[i] = Ap[i] + Ls[i] * (alf * Ue[i - 1] - Un[i - 1]);
        Un[i] = An[i] / Lp[i];
        Ue[i] = (Ae[i] - alf * Ls[i] * Ue[i - 1]) / Lp[i];
    }

    for (int i = 129; i <= 128 * 127; i++) 
    {
        Ls[i] = As[i] / (1 + alf * Ue[i - 1]);
        Lw[i] = Aw[i] / (1 + alf * Un[i - 128]);
        Lp[i] = Ap[i] + Lw[i] * (alf * Un[i - 128] - Ue[i - 128]) + Ls[i] * (alf * Ue[i - 1] - Un[i - 1]);
        Un[i] = (An[i] - alf * Lw[i] * Un[i - 128]) / Lp[i];
        Ue[i] = (Ae[i] - alf * Ls[i] * Ue[i - 1]) / Lp[i];
    }
    Ls[128 * 127 + 1] = As[128 * 127 + 1] / (1 + alf * Ue[128 * 127]);
    Lw[128 * 127 + 1] = Aw[128 * 127 + 1] / (1 + alf * Un[128 * 127 + 1 - 128]);
    Lp[128 * 127 + 1] = Ap[128 * 127 + 1] + Lw[128 * 127 + 1] * (alf * Un[128 * 127 + 1 - 128] - Ue[128 * 127 + 1 - 128]) + Ls[128 * 127 + 1] * (alf * Ue[128 * 127] - Un[128 * 127]);
    Un[128 * 127 + 1] = (An[128 * 127 + 1] - alf * Lw[128 * 127 + 1] * Un[128 * 127 + 1 - 128]) / Lp[128 * 127 + 1];
    Ue[128 * 127 + 1] = 0.0;

    for (int i = 128 * 127 + 2; i <= 128 * 128 - 1; i++) 
    {
        Ls[i] = As[i];
        Lw[i] = Aw[i] / (1 + alf * Un[i - 128]);
        Lp[i] = Ap[i] + Lw[i] * (alf * Un[i - 128] - Ue[i - 128]) + Ls[i] * (-Un[i - 1]);
        Un[i] = (An[i] - alf * Lw[i] * Un[i - 128]) / Lp[i];
        Ue[i] = 0.0;
    }
    Ls[128 * 128] = As[128 * 128];
    Lw[128 * 128] = Aw[128 * 128] / (1 + alf * Un[128 * 128 - 128]);
    Lp[128 * 128] = Ap[128 * 128] + Lw[128 * 128] * (alf * Un[128 * 127] - Ue[128 * 127]) + Ls[128 * 128] * (-Un[128 * 128 - 1]);
    Un[128 * 128] = 0.0;
    Ue[128 * 128] = 0.0;
}
...
int main() 
{
    double Ap[17000], An[17000], As[17000], Ae[17000], Aw[17000];
    double Lp[17000], Ls[17000], Lw[17000];
    double Up[17000], Un[17000], Ue[17000];
    double Lp_SIP[17000], Ls_SIP[17000], Lw_SIP[17000], Un_SIP[17000], Ue_SIP[17000];
    double alf = 0.5;
    ...
    vector_B(b);
    vector_Ap(Ap);
    vector_An_As_Ae_Aw(An, As, Ae, Aw);
    compute_SIP_Lp_Lw_Ls_Un_Ue(alf, Lp_SIP, Ls_SIP, Lw_SIP, Un_SIP, Ue_SIP, Ap, An, As, Ae, Aw);
    compute_Ad(AT, Ap, An, As, Ae, Aw, T);
    compute_residual(res, b, AT);
    construct_R(R, res, Lp_SIP, Ls_SIP, Lw_SIP);
    compute_searchdir(d, R, Un_SIP, Ue_SIP);
    ofstream outfile("SIP_Results.txt");

    do {
        
        compute_Ad(Ad, Ap, An, As, Ae, Aw, d);
        construct_R(R, res, Lp_SIP, Ls_SIP, Lw_SIP);
        compute_searchdir(Pin_res, R, Un_SIP, Ue_SIP);
        ...
        construct_R(R, res_upd, Lp_SIP, Ls_SIP, Lw_SIP);
        compute_searchdir(Pin_res_upd, R, Un_SIP, Ue_SIP);
    ...
    outfile << "Iterations = " << iter << endl;
    cout << "Results printed in SIP_Results.txt" << endl;
    return 0;
}