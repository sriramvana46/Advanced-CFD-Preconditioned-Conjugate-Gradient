...
double compute_alpha( double res[17000], double d[17000], double Ad[17000])
{
    double alpha, dtAd, restres;
...
double compute_beta(double res[17000], double res_upd[17000])
{
    double a,b,beta;
    a = compute_vectormultiplication(res,res);
    b = compute_vectormultiplication(res_upd,res_upd);
    beta = b/a;
    return beta;
}
...
int main()
{
    ...
    ofstream outfile("CGmethod_Results.txt");

    do
    {
        compute_Ad(Ad,d);
        alpha = compute_alpha( res, d, Ad);
        ...
        beta = compute_beta(res,res_upd);
        compute_do(res,res_upd);
        update_search_dir(d, res,beta);
        iter++;
        restres = compute_vectormultiplication(res,res);
    ...
    outfile << "Iterations = " << iter << endl;
    cout << "Results printed in CGmethod_Results.txt" << endl;
    return 0;
